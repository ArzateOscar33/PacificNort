...
(function (global) {
  ...
  let display = { moneda: "MXN", tipoCambio: 17.0 }; // preferencia de visualización
  ...
  function buildUrlDesgloseFisico(operacionId, idFisico) {
    return (
      `${base_url}Operaciones_maritimas_resumen/costos_desglosados_contenedor_fisico` +
  ...
  function buildUrlDesgloseOperacion(operacionId) {
    return (
      `${base_url}Operaciones_maritimas_resumen/costos_desglosados_operacion` +
      `?operacion_id=${encodeURIComponent(operacionId)}`
    );
  }
