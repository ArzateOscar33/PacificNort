...
function renderSugerenciasResumen(itemsResumen) {
  if (!Array.isArray(itemsResumen) || itemsResumen.length === 0) {
...
function doSearchSugerenciasResumen(termResumen) {
  ...
  httpResumen.open(
    "GET",
    base_url +
      "Operaciones_maritimas_resumen/sugerencias?term=" +
      encodeURIComponent(termResumen),
    true,
  );
  httpResumen.onreadystatechange = function () {
    ...
    if (this.status === 200) {
      let resResumen;
      try {
        resResumen = JSON.parse(this.responseText);
      } catch {
        resResumen = null;
      }
...
inpBuscarOpResumen.addEventListener("input", function () {
  ...
  debounceTimerResumen = setTimeout(
    () => doSearchSugerenciasResumen(termResumen),
    250,
  );
});
...
function cargarContenedoresResumen(operacionIdResumen) {
  ...
  httpResumen.open(
    "GET",
    base_url +
      "Operaciones_maritimas_resumen/listarContenedoresPorOperacion?id_operacion=" +
...
function renderContenedoresResumen(resResumen) {
  ...
  const dataResumen = Array.isArray(resResumen.contenedores)
    ? resResumen.contenedores
    : Array.isArray(resResumen.data)
      ? resResumen.data
      : [];

  if (!dataResumen || dataResumen.length === 0) {
    setContenedoresEmptyResumen();
    return;
  }
...
function consultarDetallesContenedorResumen() {
  ...
  const tipoFMResumen = mapTipoToFMResumen(
    tipoUIResumen,
    optResumen.dataset.fm,
  ); // 'M' | 'F'
  ...
  const numeroResumen =
    optResumen.dataset.numero || optResumen.textContent || "—";
  ...
  document
    .getElementById("bloqueMaritimo")
    .classList.toggle("d-none", !esMaritimoResumen);
  document
    .getElementById("bloqueFerro")
    .classList.toggle("d-none", esMaritimoResumen);
  ...
  if (tipoUI.startsWith("FERRO")) {
    ...
    if (listaCostos)
      listaCostos.innerHTML =
        '<li class="list-group-item text-muted">No aplica (Marítimo)</li>';
    CostosChart.update({
      tipo: "M",
      operacionId: Number(operacionIdActivoResumen),
    });
  ...
  const httpResumen = new XMLHttpRequest();
  const urlResumen =
    `${base_url}Operaciones_maritimas_resumen/detalles_contenedor` +
  ...
  httpResumen.onreadystatechange = function () {
    ...
    if (this.status === 200) {
      let resDetResumen;
      try {
        resDetResumen = JSON.parse(this.responseText);
      } catch {
        resDetResumen = null;
      }
      if (
        resDetResumen &&
        resDetResumen.status === "ok" &&
        resDetResumen.data
      ) {
        pintarDetalleContenedorResumen(
          esMaritimoResumen ? "MARITIMO" : "FERRO",
          resDetResumen.data,
        );
      }
    }
  ...
  cargarFaltantesResumen(
    operacionIdActivoResumen,
    tipoFMResumen,
    idPivotResumen,
    etiquetaTextoResumen,
  );
  // Cargar progreso eventos (para la card)
  fetchEventosProgresoResumen(
    operacionIdActivoResumen,
    tipoFMResumen,
    idPivotResumen,
  );
...
function pintarDetalleContenedorResumen(tipoResumen, dataResumen) {
  if (tipoResumen === "MARITIMO") {
    ...
    document.getElementById("comentarioContenedor").textContent =
      dataResumen.comentarios || "—";
  } else {
    document.getElementById("nombreContenedorResumen").textContent =
      dataResumen.numero_ferro || "—";
    document.getElementById("arriboPuerto").textContent =
      dataResumen.arribo_puerto || "Falta Registrar Arribo";
    document.getElementById("bultos").textContent =
      dataResumen.bultos != null ? dataResumen.bultos : "—";
    document.getElementById("comentarioContenedor").textContent =
      dataResumen.comentarios || "—";
  }
}
...
function toggleDFResumen(
  loadingResumen = false,
  hasDataResumen = false,
  emptyResumen = false,
) {
...
function setDFHeaderResumen(infoTextoResumen, countResumen = 0) {
  dfContenedorInfoResumen.textContent =
    infoTextoResumen || "Seleccione un contenedor…";
...
function escapeHtmlResumen(sResumen) {
  return String(sResumen ?? "").replace(
    /[&<>"']/g,
    (m) =>
      ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" })[
        m
...
function mapTipoToFMResumen(tipoUIResumen, fmResumen) {
  ...
  if (tResumen === "M" || tResumen === "MARITIMO" || tResumen === "MARÍTIMO")
    return "M";
  if (
    tResumen === "F" ||
    tResumen === "FERRO" ||
...
function cargarFaltantesResumen(
  operacionIdResumen,
  tipoFMResumen,
  idPivotResumen,
  etiquetaTextoResumen,
  ...
  const urlResumen =
    `${base_url}Operaciones_maritimas_resumen/faltantes` +
  ...
  httpResumen.onreadystatechange = function () {
    ...
    if (this.status === 200) {
      let dataResumen;
      try {
        dataResumen = JSON.parse(this.responseText);
      } catch {
        dataResumen = null;
      }
...
function setTotalCostos(v) {
  if (badgeTotalCostos) badgeTotalCostos.textContent = String(v ?? "—");
}
...
function buildUrlEventosProgresoResumen(operacionId, tipoUi, idBase) {
  // tipoUi: 'M' | 'F'
  return (
    `${base_url}Operaciones_maritimas_resumen/eventos_progreso` +
    `?operacion_id=${encodeURIComponent(operacionId)}` +
...
function fetchEventosProgresoResumen(operacionId, tipoUi, idBase) {
  ...
  xhr.open(
    "GET",
    buildUrlEventosProgresoResumen(operacionId, tipoUi, idBase),
    true,
  );
  xhr.onreadystatechange = function () {
    if (xhr.readyState !== 4) return;
