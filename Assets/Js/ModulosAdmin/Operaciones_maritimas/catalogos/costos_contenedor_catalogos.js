...
function cargarTiposMovimientoFiltroCostosContenedor(){
  ...
  http.onreadystatechange = function(){
    ...
    if (this.status !== 200) { console.warn("No se pudo cargar tipos de movimiento (filtro)"); return; }

    let tipos = [];
    try { tipos = JSON.parse(this.responseText) || []; } catch { return; }
...
perPageSelectCostosContenedor?.addEventListener("change", () => {
  perPageCostosContenedor = parseInt(perPageSelectCostosContenedor.value || "10", 10);
...
const ccBoxSugerenciasOperaciones              = document.getElementById("costosSugerenciasOperaciones");

// Contenedor (por operación)
const ccInpContOpId                            = document.getElementById("costosContenedorContenedorId");      // contenedor_operacion_id
const ccInpContOpNombre                        = document.getElementById("costosContenedorContenedorNombre");  // input texto
const ccBoxSugerenciasContenedores             = document.getElementById("sugerenciasCostosContenedor");
...
const ccInpComentarios                         = document.getElementById("costosContenedoresComentarios");
...
function ccBuscarOperaciones(term) {
  ...
  if (ccXHR_Operaciones && ccXHR_Operaciones.readyState !== 4) ccXHR_Operaciones.abort();
  ...
  ccXHR_Operaciones.onreadystatechange = function () {
    ...
    let data;
    try { data = JSON.parse(this.responseText); }
    catch { console.error("JSON inválido (ops):", this.responseText); ccOcultarSugerencias(ccBoxSugerenciasOperaciones); return; }
    ...
    data.forEach((op) => {
      ...
      btn.onclick = () => {
        ...
        if (ccInpOperacionNombre) ccInpOperacionNombre.value = op.numero_operacion || "";
...
function ccBuscarContenedoresPorOperacion(operacionId, term) {
  ...
  if (!operacionId || operacionId <= 0) {
    console.warn("[CostosContenedor] operacion_id vacío. Selecciona una operación primero.");
    ccOcultarSugerencias(ccBoxSugerenciasContenedores);
    return;
  }
  ...
  if (ccXHR_Contenedores && ccXHR_Contenedores.readyState !== 4) ccXHR_Contenedores.abort();
  ...
  ccXHR_Contenedores.onreadystatechange = function () {
    ...
    console.log("[CostosContenedor] status:", this.status, "resp:", this.responseText); // 👈
    ...
    let data;
    try { data = JSON.parse(this.responseText); }
    catch { console.error("JSON inválido (conts):", this.responseText); ccOcultarSugerencias(ccBoxSugerenciasContenedores); return; }
...
document.addEventListener("click", function (e) {
  if (ccBoxSugerenciasOperaciones && !ccInpOperacionNombre?.contains(e.target) && !ccBoxSugerenciasOperaciones.contains(e.target)) {
    ccOcultarSugerencias(ccBoxSugerenciasOperaciones);
  }
  if (ccBoxSugerenciasContenedores && !ccInpContOpNombre?.contains(e.target) && !ccBoxSugerenciasContenedores.contains(e.target)) {
    ccOcultarSugerencias(ccBoxSugerenciasContenedores);
  }
});
