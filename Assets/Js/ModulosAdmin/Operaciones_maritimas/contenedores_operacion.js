...
// Fechas
const inpFechaDesdeCont = document.getElementById(
  "fechaDesdeContenedoresEnOperacion",
);
const inpFechaHastaCont = document.getElementById(
  "fechaHastaContenedoresEnOperacion",
);
...
[inpFechaDesdeCont, inpFechaHastaCont].forEach((el) => {
  el?.addEventListener("change", () => {
    ...
    if (
      inpFechaDesdeCont.value &&
      inpFechaHastaCont.value &&
...
function listarContenedores() {
  ...
  const url =
    base_url +
    "Operaciones_maritimas_contenedores/listar" +
    (params.toString() ? "?" + params.toString() : "");

  if (
    currentListXHRContenedores &&
    currentListXHRContenedores.readyState !== 4
  ) {
...
(function () {
  ...
  function pick(hid, inp, box, id, label) {
    if (hid) hid.value = id;
...
(function () {
  ...
  form?.addEventListener("submit", function (e) {
    ...
    const btnGuardar = document.querySelector(
      'button[form="formAgregarContenedor"]',
    );
    ...
    const operacion_id =
      parseInt(
        document.getElementById("operacion_id")?.value?.trim() || "0",
        10,
      ) || 0;
    let numero_ferro =
      document.getElementById("contenedorNombre")?.value?.trim() || "";
    const bultos =
      document.getElementById("bultosContenedores")?.value?.trim() || "";
    const cliente_id =
      document.getElementById("cliente_id")?.value?.trim() || "";
    const comentarios =
    ...
    if (!operacion_id) {
      Swal.fire("Aviso", "Selecciona una operación", "warning");
      return;
    }
    if (!cliente_id) {
      Swal.fire("Aviso", "La operación no tiene cliente asignado", "warning");
      return;
    }
    if (!numero_ferro) {
      Swal.fire(
        "Aviso",
        "Captura o selecciona el contenedor físico",
        "warning",
      );
      return;
    }
    ...
    if (
      (modo === "create" || !row_id) &&
      ferroYaEnTabla(operacion_id, numero_ferro)
    ) {
      Swal.fire(
        "Aviso",
        "Este contenedor físico ya está en la operación (vista actual)",
        "warning",
      );
      return;
    }
    ...
    http.onreadystatechange = function () {
      ...
      if (res.status === "warning") {
        Swal.fire(
          "Aviso",
          res.msg || "No se pudo completar la acción",
          "warning",
        );
...
tablaContenedores.addEventListener("click", function (e) {
  ...
  if (!rowId || !tipo) {
    Swal.fire(
      "Aviso",
      "No se pudo identificar el contenedor a editar",
      "warning",
    );
    return;
  }
  ...
  if (tipo === "maritimo") {
    Swal.fire(
      "Aviso",
      "Contenedor Marítimo se edita en Módulo de Operaciones",
      "warning",
    );
    return;
  }
  ...
  const url =
    base_url +
    "Operaciones_maritimas_contenedores/detalle?tipo=" +
  ...
  x.onreadystatechange = function () {
    ...
    if (res.status === "warning" && res.data?.editable === false) {
      Swal.fire(
        "Aviso",
        res.msg || "Este contenedor se edita en el módulo de Operaciones",
        "warning",
      );
      return;
    }
    ...
    if (title)
      title.innerHTML =
        '<i data-feather="edit" class="me-1"></i> Editar Contenedor (Terrestre)';
    const btnGuardar = document.querySelector(
      'button[form="formAgregarContenedor"]',
    );
    if (btnGuardar)
      btnGuardar.innerHTML = '<i data-feather="save"></i> Actualizar';
...
// Excel
document
  .getElementById("btnExportarExcelContenedoresOperacion")
...
// PDF
document
  .getElementById("btnExportarPDFContenedoresOperacion")
  ?.addEventListener("click", () => {
    ExportarTablas.exportar({
