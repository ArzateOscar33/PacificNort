...
// ------- Refs del DOM -------
const tbodyCostosContenedor = document.getElementById(
  "tbodyCostosContenedores",
);
const inputBuscarCostosContenedor = document.getElementById(
  "buscarCostoContenedor",
);
const selMonedaCostosContenedor = document.getElementById(
  "filtroMonedaCostoContenedor",
);
const selTipoCostoContenedor = document.getElementById(
  "filtroTipoCostoContenedor",
);
...
const metaResumenCostosContenedor =
  document.getElementById("metaResumenCostos");
const costosContenedorTiposMap = Object.create(null);
// ------- Estado -------
let currentPageCostosContenedor = 1;
let perPageCostosContenedor = parseInt(
...
function listarCostosContenedor(page = 1) {
  ...
  const naturaleza = (
    document.getElementById("filtroNaturalezaCostoContenedor")?.value || ""
  ).toUpperCase();

  const buscar = (inputBuscarCostosContenedor?.value || "").trim();
  const moneda = normalizeMonedaCostosContenedor(
    selMonedaCostosContenedor?.value || "",
  );
  const tipoId = parseInt(selTipoCostoContenedor?.value || "0", 10) || 0;

  // Cancelar XHR en vuelo
  if (
    currentXHRCostosContenedor &&
    currentXHRCostosContenedor.readyState !== 4
  ) {
  ...
  currentXHRCostosContenedor.onreadystatechange = function () {
    ...
    try {
      ...
      renderErrorCostosContenedor("Respuesta inválida del servidor.");
      return;
    }
    ...
    const meta = payload.meta || {
      ...
      totalPages: 1,
      total: 0,
      perPage: perPageCostosContenedor,
    };
    ...
    if (
      data.length === 0 &&
      meta.totalPages > 0 &&
      currentPageCostosContenedor > meta.totalPages
    ) {
...
const costosContenedorModal = costosContenedorModalEl
  ? new bootstrap.Modal(costosContenedorModalEl)
  : null;
const costosContenedorForm = document.getElementById(
  "formAgregarCostoContenedores",
);
const costosContenedorBtnNuevo = document.getElementById(
  "btnNuevoCostoContenedor",
);

// Operación
const costosContenedorInpOpId = document.getElementById("costosOperacionid");
const costosContenedorInpOpNombre = document.getElementById(
  "costosOperacionNombre",
);
const costosContenedorBoxOps = document.getElementById(
  "costosSugerenciasOperaciones",
);

// Contenedor (en operación)
const costosContenedorInpContOpId = document.getElementById(
  "costosContenedorContenedorId",
);
const costosContenedorInpContOpNombre = document.getElementById(
  "costosContenedorContenedorNombre",
);
const costosContenedorBoxConts = document.getElementById(
  "sugerenciasCostosContenedor",
);

// Tipo / Moneda / Monto / Comentarios
const costosContenedorSelTipo = document.getElementById(
  "costosContenedoresTipoCosto",
);
const costosContenedorSelMoneda = document.getElementById(
  "costosContenedoresMoneda",
);
const costosContenedorInpMonto = document.getElementById(
  "costosContenedoresMonto",
);
const costosContenedorInpComentarios = document.getElementById(
  "costosContenedoresComentarios",
);
const costosContenedorBtnSubmit = document.getElementById(
  "btnSubmitCostoContenedor",
);
...
function costosContenedorCargarTiposMovimiento(done) {
  ...
  xhr.onreadystatechange = function () {
    ...
    for (const k in costosContenedorTiposMap)
      delete costosContenedorTiposMap[k];
    ...
    data.forEach((t) => {
      ...
      const mon =
        String(t.moneda || "").toUpperCase() === "DLLS" ? "DLLS" : "PESOS";
...
costosContenedorSelTipo?.addEventListener("change", () => {
  if (!costosContenedorSelMoneda) return;
  const sel =
    costosContenedorSelTipo.options[costosContenedorSelTipo.selectedIndex];
  // usa dataset o, si no viene, el mapa
  const mon =
    sel?.dataset?.moneda ||
    costosContenedorTiposMap[costosContenedorSelTipo.value] ||
    "";
...
costosContenedorBtnNuevo?.addEventListener("click", () => {
  ...
  if (costosContenedorBoxOps) {
    costosContenedorBoxOps.innerHTML = "";
    costosContenedorBoxOps.style.display = "none";
  }
  if (costosContenedorBoxConts) {
    costosContenedorBoxConts.innerHTML = "";
    costosContenedorBoxConts.style.display = "none";
  }
...
costosContenedorForm?.addEventListener("submit", (e) => {
  ...
  if (contOpId <= 0) {
    Swal.fire(
      "Aviso",
      "Selecciona una operación y un contenedor válido.",
      "warning",
    );
    costosContenedorInpOpNombre?.focus();
    return;
  }
  if (tipoId <= 0) {
    Swal.fire("Aviso", "Selecciona un tipo de costo.", "warning");
    costosContenedorSelTipo?.focus();
    return;
  }
  if (montoVal <= 0) {
    Swal.fire("Aviso", "El monto debe ser mayor a 0.", "warning");
    costosContenedorInpMonto?.focus();
    return;
  }
...
window.ccEditarCostoContenedor = function (id) {
  ...
  xhr.onreadystatechange = function () {
    ...
    if (res.status !== "success" || !res.data) {
      Swal.fire("Aviso", res.msg || "Registro no encontrado.", "warning");
      return;
    }
    ...
    costosContenedorInpContOpId.value =
      d.id_contenedor_operacion || d.contenedor_operacion_id || "";
    ...
    costosContenedorCargarTiposMovimiento(() => {
      ...
      if (costosContenedorSelMoneda) {
        const sel =
          costosContenedorSelTipo.options[
            costosContenedorSelTipo.selectedIndex
          ];
        const mon =
          sel?.dataset?.moneda || String(d.moneda || "").toUpperCase();
        costosContenedorSelMoneda.value = mon || "";
...
window.ccEliminarCostoContenedor = function (id) {
  ...
  Swal.fire({
    ...
    xhr.open(
      "POST",
      `${base_url}Operaciones_maritimas_costos_Contenedor/eliminarCostoContenedor`,
      true,
    );
    xhr.send(fd);
    xhr.onreadystatechange = function () {
      ...
      if (res.status === "success") {
        ...
        Swal.fire(
          "Eliminado",
          res.msg || "Costo eliminado correctamente.",
          "success",
        );
      } else {
        Swal.fire(
...
// Excel
document
  .getElementById("btnExportarExcelCostosContenedor")
...
// PDF
document
  .getElementById("btnExportarPDFCostosContenedor")
  ?.addEventListener("click", () => {
    ExportarTablas.exportar({
