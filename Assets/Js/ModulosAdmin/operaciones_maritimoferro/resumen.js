...
function doSearchSugerenciasResumen(termResumen) {
  ...
  httpResumen.open(
    "GET",
    base_url + "operaciones_maritimo_ferro_resumen/sugerencias?term=" + encodeURIComponent(termResumen),
    true
  );
...
function renderContenedoresResumen(resResumen) {
  ...
  dataResumen.forEach(cResumen => {
    ...
    const tipoUI = (cResumen.tipo_contenedor || '').toUpperCase(); // "MARITIMO" | "FERRO"

    // ✅ value = ID BASE (id_contenedor_maritimo | id_fisico)
    optionResumen.value = cResumen.id_contenedor ?? '';

    optionResumen.textContent = `${cResumen.tipo_contenedor} · ${cResumen.numero_contenedor}`;

    optionResumen.dataset.tipo    = tipoUI;
    optionResumen.dataset.fm      = (cResumen.fm_tipo || '').toUpperCase();
    optionResumen.dataset.numero  = cResumen.numero_contenedor || '';
    optionResumen.dataset.baseId  = cResumen.id_contenedor ?? ''; // ID BASE

    // Para costos de contenedor físico
    if (tipoUI === 'FERRO') {
...
function consultarDetallesContenedorResumen() {
  ...
  if (tipoUI.startsWith('FERRO')) {
    ...
    setTotalCostos('—');
    if (listaCostos) {
      listaCostos.innerHTML = '<li class="list-group-item text-muted">No aplica (Marítimo)</li>';
    }
    if (window.CostosChart) {
      CostosChart.update({
        tipo: 'M',
  ...
  // === 2) FALTANTES (usa ID BASE) ===
  const etiquetaTextoResumen = `${esMaritimoResumen ? 'Contenedor' : 'Ferro'} ${numeroResumen}`;
  cargarFaltantesResumen(operacionIdActivoResumen, tipoFMResumen, idBaseResumen, etiquetaTextoResumen);

  // === 3) PROGRESO DE EVENTOS (usa ID BASE) ===
  fetchEventosProgresoResumen(operacionIdActivoResumen, tipoFMResumen, idBaseResumen);

  // === 4) EVENTOS (tabla + timeline) → también ID BASE ===
...
function setDFHeaderResumen(infoTextoResumen, countResumen=0){
  ...
  docsPendientesResumen.textContent   = String(countResumen || 0);
}
...
function cargarFaltantesResumen(operacionIdResumen, tipoFMResumen, idBaseResumen, etiquetaTextoResumen){
  if (!operacionIdResumen || !tipoFMResumen || !idBaseResumen){
    setDFHeaderResumen('Seleccione un contenedor…', 0);
    toggleDFResumen(false, false, false);
    dfListaResumen.innerHTML = '';
    return;
...
function fetchCostosTotalesFisico(operacionId, idFisico){
  setTotalCostos('…');
  const url = `${base_url}operaciones_maritimo_ferro_resumen/costos_totales_contenedor_fisico`
    + `?operacion_id=${encodeURIComponent(operacionId)}&id_fisico=${encodeURIComponent(idFisico)}`;
...
function fetchCostosDesglosadosFisico(operacionId, idFisico){
  ...
  const url = `${base_url}operaciones_maritimo_ferro_resumen/costos_desglosados_contenedor_fisico`
    + `?operacion_id=${encodeURIComponent(operacionId)}&id_fisico=${encodeURIComponent(idFisico)}`;
  const xhr = new XMLHttpRequest();
  xhr.open('GET', url, true);
  xhr.onreadystatechange = function(){
...
// ====== Eventos logísticos (tabla + timeline) ======
...
function cargarEventosResumen(operacionId, tipoUi, idBase){
  ...
  xhr.onreadystatechange = function(){
    ...
    if (r.status === 'ok' && Array.isArray(r.data)) {
      ...
      if (window.TimelineChart) TimelineChart.setEventos(r.data);
    } else {
      setEventosEmptyResumen();
      if (window.TimelineChart) TimelineChart.setEventos([]);
    }
