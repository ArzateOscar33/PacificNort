...
(function () {
  ...
  window.editarFerroOP = function (idRow) {
    ...
    x.open(
      "GET",
      BASE_URL +
        "Operaciones_maritimo_ferro_contenedores/obtener?id=" +
        encodeURIComponent(idRow),
      true,
    );
    x.onload = function () {
      if (x.status !== 200) {
      ...
      const contFerroNom = document.getElementById(
        "contenedorFerroNombreFerroOP",
      );
      ...
      if (
        ...
        typeof window.setCarritoFerroOP === "function"
      ) {
    ...
    x.onerror = function () {
      console.error("Error de red al obtener detalle");
    };
    x.send();
  };
...
(function () {
  ...
  const contNameInp = document.getElementById(
    "contenedorMaritimoNombreFerroOP",
  );
  ...
  document
    .getElementById("modalFerroOP")
    ?.addEventListener("shown.bs.modal", syncReadonlyFerro);
  document
    .getElementById("modalFerroOP")
    ?.addEventListener("hidden.bs.modal", syncReadonlyFerro);
  ...
  window.getCarritoFerroOP = function () {
    return carrito;
  };
  ...
  function actualizarTotales() {
    const totBultos = carrito.reduce(
      (a, b) => a + Number(b.bultos_asignados || 0),
      0,
    );
    ...
    if (badgeTotB) {
      ...
      badgeTotB.className =
        "badge " +
        (totBultos > 0 ? "bg-success text-white" : "bg-secondary text-white");
    }
    if (badgeTotM) {
      ...
      badgeTotM.className =
        "badge " +
        (totMar > 0 ? "bg-success text-white" : "bg-secondary text-white");
    }
  }
  function limpiarLinea() {
    ...
    // Estado visual
    if (badgeSaldo) {
      badgeSaldo.textContent = "Saldo: 0";
      badgeSaldo.className = "badge bg-secondary text-white";
    }
    if (opInp) {
      opInp.dataset.lastPick = "0";
    } // marcar que no hay pick activo
    ...
    if (typeof window.toggleSelectorMaritimo === "function")
      window.toggleSelectorMaritimo(true);
  }
  ...
  if (btnConfirmar && !btnConfirmar.dataset.bound) {
    ...
    btnConfirmar.addEventListener("click", function () {
      ...
      const cliente =
        document.getElementById("clienteNombreMaritimoFerroOP")?.value || "";

      const ix = carrito.findIndex((x) => Number(x.cmo_id) === cmoId);
      if (ix >= 0)
        carrito[ix].bultos_asignados =
          Number(carrito[ix].bultos_asignados) + asign;
      else
        carrito.push({
          cmo_id: cmoId,
  ...
  function onSubmitFerro(e) {
    ...
    if (wasMultiple) {
      const totalBultos = carrito.reduce(
        (sum, item) => sum + Number(item.bultos_asignados || 0),
        0,
      );
    ...
    const fxId = Number(
      document.getElementById("contenedorFerroIdFerroOP")?.value || 0,
    );
    const fxNm = (
    ...
    if (!fxId && !fxNm)
      return toast("Selecciona la caja/ferro o escribe el número.", false);
    ...
    if (!wasMultiple) {
      ...
      const cmoIdLocal = Number(
        document.getElementById("contMaritimoOperacionIdFerroOP")?.value || 0,
      );
      const asigLocal = parseInt(
        (bultosAsignadosFerroOP?.value || "").trim(),
        10,
      );
      const restLocal = Number(bultosRestantesFerroOP?.value || 0);

      if (!cmoIdLocal)
        return toast("Selecciona una operación/CMO válido.", false);
      if (!Number.isFinite(asigLocal) || asigLocal <= 0)
        return toast("Los bultos asignados deben ser > 0.", false);
      if (asigLocal > restLocal)
        return toast(
          `No hay saldo suficiente. Disponible: ${restLocal}.`,
          false,
        );
    } else {
      ...
      if (carrito.length === 0) {
        return toast(
          "Agrega al menos un contenedor marítimo al carrito.",
          false,
        );
      }
    ...
    function postAsignacion() {
      ...
      x.onload = function () {
        ...
        if (!res || res.ok !== true) {
          const errorMsg =
            res && res.msg ? res.msg : "No se pudo registrar la asignación.";
  ...
  ["input", "change"].forEach((ev) => {
    ...
    document
      .getElementById("operacionMaritimaNombreFerroOP")
      ?.addEventListener(ev, toggleBtn);
  });
  toggleBtn();
  ...
  window.resetModalFerroOP = function resetModalFerroOP() {
    ...
    [
      "sugOperacionesMaritimasFerroOP",
      "sugFerrosFerroOP",
      "sugTransportistasFerroOP",
      "destinoFerroOP",
    ].forEach((id) => {
    ...
    if (badgeB) {
      ...
      badgeB.className = "badge bg-secondary text-white";
    }
    if (badgeM) {
      badgeM.textContent = "0";
      badgeM.className = "badge bg-secondary text-white";
    }
    if (badgeSaldo) {
      badgeSaldo.textContent = "Saldo: 0";
      badgeSaldo.className = "badge bg-secondary text-white";
