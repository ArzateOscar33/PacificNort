...
btnAgregarUsuario.addEventListener("click", () => {
  ...
  document.getElementById("modalRegistrarUsuarioLabel").textContent = "Registrar Usuario";
...
selDepto.addEventListener("change", function () {
  ...
  const http = new XMLHttpRequest();
  http.open("GET", base_url + "Usuarios/puestosPorDepartamento/" + encodeURIComponent(depId), true);
  http.send();
...
function editarUsuario(id) {
  ...
  http.onreadystatechange = function () {
    if (this.readyState === 4 && this.status === 200) {
      ...
      document.querySelector('select[name="active"]').value = (typeof data.estatus !== "undefined" ? data.estatus : 1);

      // Departamento -> cargar puestos y seleccionar el del usuario
      const selDepto = document.getElementById("departamento_id");
      const selPuesto = document.getElementById("puesto_id");

      selDepto.value = data.departamento_id || "";
      // Cargar puestos del depto y luego seleccionar el puesto_id
      selPuesto.innerHTML = '<option value="">Cargando...</option>';
      selPuesto.disabled = true;

      const http2 = new XMLHttpRequest();
      http2.open("GET", base_url + "Usuarios/puestosPorDepartamento/" + encodeURIComponent(selDepto.value), true);
      http2.send();
      http2.onreadystatechange = function () {
        if (http2.readyState === 4 && http2.status === 200) {
          let puestos = [];
          try { puestos = JSON.parse(http2.responseText); } catch (e) {}
          ...
          selPuesto.disabled = false;
          selPuesto.value = data.puesto_id || "";
...
document.getElementById("btnAgregarUsuario").addEventListener("click", () => {
  ...
  document.getElementById("modalRegistrarUsuarioLabel").textContent = "Registrar Usuario";
  const btn = document.querySelector('#modalRegistrarUsuario button[type="submit"]');
  if (btn) btn.innerHTML = '<i data-feather="check-circle" class="me-1"></i> Agregar';
  feather.replace();
...
inputBuscar?.addEventListener("keyup", function () {
  ...
  const http = new XMLHttpRequest();
  http.open("GET", base_url + "Usuarios/buscar?term=" + encodeURIComponent(term), true);
  http.send();
  http.onreadystatechange = function () {
    if (this.readyState === 4 && this.status === 200) {
      let data;
