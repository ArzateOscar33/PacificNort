 ...
 * ========================= */
    public function kpisPortalCliente(int $clienteId): array
    {
        if ($clienteId <= 0) {
            return [
                'mar_agua'    => 0,
                'mar_puerto'  => 0,
                'fo_camino'   => 0, // puedes renombrarlo después si quieres
                'entregadas'  => 0,
                'bodegas'     => 0,
                'yardas'      => 0,
            ];
        ...
        return [
            // En agua
            'mar_agua'   => $this->contarOperacionesPorEstatus($clienteId, 9),

            // Puerto
            'mar_puerto' => $this->contarOperacionesPorEstatus($clienteId, 11),

            // Terrestres en camino = CAMINO A DESTINO
            'fo_camino'  => $this->contarOperacionesPorEstatus($clienteId, 1),

            // Entregadas
            'entregadas' => $this->contarOperacionesPorEstatus($clienteId, 7),

            // Bodegas = BODEGA MX + BODEGA USA
            'bodegas'    => $this->contarOperacionesPorEstatusIds($clienteId, [5, 6]),

            // Yardas = YARDA MX + YARDA USA
            'yardas'     => $this->contarOperacionesPorEstatusIds($clienteId, [10, 12]),
        ];
    }

    /**
     * Cuenta operaciones del cliente por estatus.
     * Ahora todo sale de la operación marítima maestra.
     */
    public function contarOperacionesPorEstatus(int $clienteId, int $estatusId): int
    {
    ...
    /**
     * Cuenta operaciones del cliente por múltiples estatus.
     * Ahora todo sale de la operación marítima maestra.
     */
    public function contarOperacionesPorEstatusIds(int $clienteId, array $estatusIds): int
    {
    ...
    public function listarDocumentosOperacionPortal(
        ...
        SELECT
            d.id_documento,
            o.numero_operacion,
            COALESCE(cm.numero_contenedor,'') AS contenedor,
    ...
    public function listarTiposDocumentoPorSubtipo(int $subtipoId): array
    {
        ...
        // Portal clientes: SOLO MAR(1) y LBMF(11)
        if (!in_array($tipoOperacionId, [1, 11], true)) return [];

        // 2) En portal marítimo solo permitimos estos aplica_sobre
        // (si LBMF también usa docs de contenedor_fisico, NO lo incluimos porque pediste solo marítimo)
    ...
    private function getSubtipoOperacionIdPorOperacion(string $tipo, int $operacionId): int
    {
        ...
        $tipo = strtoupper(trim($tipo));
        if (!in_array($tipo, ['MAR', 'LBMF'], true)) return 0;
    ...
    {
        $tipo = strtoupper(trim($tipo));
        if (!in_array($tipo, ['MAR', 'LBMF'], true)) return [];
    ...
    public function insertarDocumentoOperacion(array $d): int
    {
        ...
        $coId  = isset($d['contenedor_operacion_id']) ? (int)$d['contenedor_operacion_id'] : 0;
        $cmoId = isset($d['cont_maritimo_operacion_id']) ? (int)$d['cont_maritimo_operacion_id'] : 0;

        if ($operacionId <= 0 || $tipoDocId <= 0 || $nombreArchivo === '' || $rutaArchivo === '') {
            return 0;
        }

        // ✅ Para no permitir docs "huérfanos" sin contenedor asociado
        if ($coId <= 0 && $cmoId <= 0) {
            return 0;
        }
