...
public function listarPorFactura(int $facturaId, string $term = "", int $estatus = 1): array
    {
        ...
        $whereTerm = "";
        if ($term !== "") {
            // Importante: escapado mínimo para LIKE (si tu Query soporta bind params, mejor aún)
            $safe = str_replace(["\\", "%", "_"], ["\\\\", "\\%", "\\_"], $term);
            $like = "'%" . $safe . "%'";

            $whereTerm = " AND (
                d.nombre_archivo LIKE $like
                OR td.nombre LIKE $like
                OR IFNULL(d.mime_type,'') LIKE $like
            ) ";
        }
    ...
    /**
     * (Opcional pero recomendado) Obtiene encabezado mínimo de la factura
     * para mostrar badge/labels cuando se seleccione en el input con sugerencias.
     */
    ...
    /**
     * Tipos de documento para el select del modal.
     * Toma en cuenta tu enum aplica_sobre y el flag activo.
     */
    ...
    public function sugerirFacturas(string $term, int $limit = 10): array
{
    ...
    $sql = "SELECT
            ...
            WHERE f.estatus = 1
              AND (
                f.numero_factura LIKE $like
                OR IFNULL(f.proveedor,'') LIKE $like
              )
            ORDER BY f.id_factura DESC
            LIMIT $limit";
