...
class Operaciones_maritimo_ferro_eventos_ferModel extends Query
{
    ...
    $sqlCount = "
      ...
      FROM (
        ...
        LEFT JOIN eventos_terrestres_observaciones obs
         ...
         AND obs.operacion_ferro_id = of.id_operacion_ferro
  ...
  private function validarRenglonEventoTerrestre(
    ...
    $row = $this->select($sql, [$operacionId, $opFerroId, $ferroId]);
    return !empty($row);
  }
  ...
  public function guardarObservacionRenglon(array $data, int $idUsuario = 0): bool
  {
    ...
    /*
      Si la observación viene vacía:
      - No se elimina físicamente.
      - Se hace baja lógica si existe activa.
    */
    if ($observacion === '') {
      $sqlDel = "
    ...
    $actual = $this->obtenerObservacionRenglon($operacionId, $opFerroId, $ferroId);
    ...
    /*
      Si ya existía una observación previa con estatus = 0,
      la reactivamos para no chocar con el UNIQUE KEY.
    */
    $sqlInactive = "
