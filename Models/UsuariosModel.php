...
class UsuariosModel extends Query
{
    public function listar()
    {
        $sql = "SELECT
            ...
            WHERE u.estatus = 1
            GROUP BY u.id_usuario, u.nombre, u.apellido, u.correo, u.telefono, p.nombre, d.nombre
    ...
    public function obtenerDepartamentoDePuesto($puestoId) {
        ...
        return $this->select($sql, [$puestoId]);
    }
    ...
    {
        $sql = "SELECT id_usuario FROM usuarios WHERE estatus = 1 AND LOWER(correo) = LOWER(?)";
        return $this->select($sql, [$correo]);
    }

    
    public function registrarUsuario($nombre, $apellido, $correo, $hash, $tel, $puestoId, $deptoId, $estatus)
    {
        $sql = "INSERT INTO usuarios (nombre, apellido, correo, clave, telefono, puesto_id, departamento_id, estatus)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        return $this->insertar($sql, [$nombre, $apellido, $correo, $hash, $tel, $puestoId, $deptoId, $estatus]);
    }

    // por si insertar no devuelve ID: lo obtenemos por correo
    public function obtenerPorCorreo($correo)
    {
        $sql = "SELECT id_usuario FROM usuarios WHERE LOWER(correo) = LOWER(?) LIMIT 1";
        return $this->select($sql, [$correo]);
    }
    ...
    // (opcional) por si quieres validar duplicado de rol
    public function tieneRol($usuarioId, $rolId)
    {
        $sql = "SELECT * FROM roles_usuario WHERE usuario_id = ? AND rol_id = ? LIMIT 1";
    ...
    public function obtenerUsuario($id)
    {
        $sql = "SELECT 
                ...
                u.departamento_id,
                u.estatus,
                (SELECT ru.rol_id FROM roles_usuario ru WHERE ru.usuario_id = u.id_usuario LIMIT 1) AS rol_id
                FROM usuarios u
                WHERE u.id_usuario = ?";
        return $this->select($sql, [$id]);
    }
    public function actualizarUsuario($id, $nombre, $apellido, $correo, $tel, $puestoId, $deptoId, $estatus, $hash = null)
    {
        if ($hash) {
            $sql = "UPDATE usuarios
                    SET nombre = ?, apellido = ?, correo = ?, telefono = ?, puesto_id = ?, departamento_id = ?, estatus = ?, clave = ?
                    WHERE id_usuario = ?";
            return $this->save($sql, [$nombre, $apellido, $correo, $tel, $puestoId, $deptoId, $estatus, $hash, $id]);
        } else {
            $sql = "UPDATE usuarios
                    SET nombre = ?, apellido = ?, correo = ?, telefono = ?, puesto_id = ?, departamento_id = ?, estatus = ?
                    WHERE id_usuario = ?";
            return $this->save($sql, [$nombre, $apellido, $correo, $tel, $puestoId, $deptoId, $estatus, $id]);
        }
    }
    ...
    public function existeCorreoOtro($correo, $idUsuario)
    {
        $sql = "SELECT id_usuario 
                FROM usuarios 
                WHERE estatus = 1 AND LOWER(correo) = LOWER(?) AND id_usuario <> ?";
    ...
    {
        // Buscamos por nombre, apellido, correo, puesto o departamento
        $sql = "SELECT
                    u.id_usuario,
                    u.nombre,
                    u.apellido,
