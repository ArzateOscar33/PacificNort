...
class Operaciones_maritimo_ferro_trazabilidad extends Controller
{
    ...
    public function ferro_por_operacion(): void
    {
        ...
        if (!$row) {
            $this->notFound('No se encontró ferro/caja para esta operación.');
        }
    ...
    public function crear_ruta_ferro(): void
    {
        ...
        // no permitir duplicar trazabilidad
        if ($this->model->operacionFerroTieneRutaActiva($opId)) {
            $this->json([
                'ok'  => false,
                'msg' => 'Esta operación ya cuenta con una trazabilidad registrada. No es posible crear otra.'
            ], 409);
        }
