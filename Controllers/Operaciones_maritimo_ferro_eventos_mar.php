...
class Operaciones_maritimo_ferro_eventos_mar extends Controller
{
    ...
    public function listar()
    {
        ...
        try {
            // SOLO MF (11) + filtros resueltos en el modelo
   ...
   ============================================================= */
    ...
    public function registrar()
    {
        ...
        if ($evento['operacion_id'] <= 0) {
            echo json_encode(['status' => 'warning', 'msg' => 'Selecciona una operación (MF).']);
            die();
        }
        if ($evento['cont_maritimo_operacion_id'] <= 0) {
            echo json_encode(['status' => 'warning', 'msg' => 'No hay contenedor marítimo ligado a la operación.']);
            die();
        }
