...
class Operaciones_maritimo_ferro_documentos extends Controller
{
    ...
    public function listar()
    {
        ...
        // Nota: el nuevo modelo trae listarDocumentosMF
        $rows = $this->model->listarDocumentosMF($operacion_id, $contenedor_id, $tipo);
    ...
    {
        header('Content-Type: application/json; charset=UTF-8');
        $this->registrarMF(); // ya no existe FO
    }
    ...
    private function registrarMF()
    {
        try {
            ...
            $contenedor_tipo = strtoupper(trim($_POST['contenedor_tipo'] ?? '')); // 'F'|'M'
            $tipo_doc_id     = (int)($_POST['tipo_documento_id'] ?? 0);

            $coId  = ($contenedor_tipo === 'F' ? $contenedor_id : null); // contenedores_operacion.id_contenedor
            $cmoId = ($contenedor_tipo === 'M' ? $contenedor_id : null); // contenedores_maritimos_operacion.id

            if (($coId && $cmoId) || (!$coId && !$cmoId)) {
                echo json_encode(['status' => 'warning', 'msg' => 'Selección de contenedor inválida']);
                return;
            }
    ...
    public function eliminar($id = 0)
    {
        ...
        try {
            ...
            $coId  = $doc['contenedor_operacion_id'] ?? null;
            $cmoId = $doc['cont_maritimo_operacion_id'] ?? null;

            $contTipo = $coId ? 'FISICO' : ($cmoId ? 'MARITIMO' : 'N/A');
            $contRef  = $coId ?: ($cmoId ?: '-');
    ...
    public function faltantes()
    {
        ...
        try {
            $rows = $this->model->faltantesMF($operacion_id, $contenedor_id, $tipo);
            echo json_encode($rows, JSON_UNESCAPED_UNICODE);
        } catch (Throwable $e) {
    ...
    public function notificarFaltantes()
    {
        ...
        try {
            ...
            $faltantes = $this->model->faltantesMF($operacion_id, $contenedor_id, $tipo);
