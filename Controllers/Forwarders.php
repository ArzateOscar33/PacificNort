...
class Forwarders extends Controller
{
    ...
    public function registrar()
    {
        ...
        if ($existe) {
            if ((int)$existe['estatus'] === 1) {
                echo json_encode(['status' => 'warning', 'msg' => 'Ya existe un forwarder con ese nombre']); die();
            }
            // Existe inactivo -> reactivar
            $ok = $this->model->reactivar($existe['id_forwarder'], $nombre, $contacto);
            if ($ok) { echo json_encode(['status' => 'success', 'msg' => 'Forwarder reactivado correctamente']); die(); }
            echo json_encode(['status' => 'error', 'msg' => 'No se pudo reactivar el forwarder']); die();
        }

        $nuevoId = $this->model->registrar($nombre, $contacto);
        if (!$nuevoId) {
            echo json_encode(['status' => 'error', 'msg' => 'No se pudo registrar el forwarder']); die();
        }

        echo json_encode(['status' => 'success', 'msg' => 'Forwarder registrado correctamente']); die();
    }
    ...
    public function actualizar()
    {
        ...
        if ($dup && (int)$dup['estatus'] === 1) {
            echo json_encode(['status' => 'warning', 'msg' => 'Ya existe otro forwarder con ese nombre']); die();
        }

        $ok = $this->model->actualizar($id_forwarder, $nombre, $contacto);
        if (!$ok) {
            echo json_encode(['status' => 'error', 'msg' => 'No se pudo actualizar el forwarder']); die();
        }

        echo json_encode(['status' => 'success', 'msg' => 'Forwarder actualizado correctamente']); die();
    }
    ...
    public function eliminar($id)
    {
        ...
        if ($ok) { echo json_encode(['status' => 'success', 'msg' => 'Forwarder desactivado correctamente']); die(); }
        echo json_encode(['status' => 'error', 'msg' => 'No se pudo desactivar el forwarder']); die();
    }
