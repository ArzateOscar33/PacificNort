...
class Dashboard extends Controller
{
    ...
    public function kpis()
    {
        try {
            ...
            // ✅ Nuevo: FO (ferro / terrestres)
            $opsFOActivas  = (int)$this->model->kpiOperacionesFOActivas();
    ...
    // ✅ Nuevo endpoint, por si lo quieres usar de forma independiente
    public function kpi_operaciones_fo()
    {
        $n = (int)$this->model->kpiOperacionesFOActivas();
        echo json_encode(['status' => 'ok', 'data' => ['value' => $n]], JSON_UNESCAPED_UNICODE);
        die();
    }
    ...
    public function costos_mensuales()
    {
        ...
        echo json_encode([
            'status' => 'ok',
            'meta'   => ['months' => $meses, 'currency' => $moneda, 'fx' => $fx],
            'data'   => $rows
        ], JSON_UNESCAPED_UNICODE);
        die();
    }
