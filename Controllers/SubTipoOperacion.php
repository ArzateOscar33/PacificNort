...
class SubTipoOperacion extends Controller
{
    ...
    public function registrar()
    {
        ...
        if (empty($nombre) || empty($clave) || empty($id_tipo_operacion)) {
            echo json_encode(['status' => "error", 'msg' => 'Nombre, clave y tipo de operación son obligatorios'], JSON_UNESCAPED_UNICODE);
            die();
        }
        ...
        if ($this->tipoRequierePuerto($id_tipo_operacion)) {
            if (empty($puerto_id_raw)) {
                echo json_encode(['status' => "error", 'msg' => 'Este tipo requiere un puerto'], JSON_UNESCAPED_UNICODE);
                die();
            }
        ...
        if ($this->model->existeSubtipoPorClave($clave)) {
            echo json_encode(['status' => "error", 'msg' => 'Ya existe un subtipo con esa clave'], JSON_UNESCAPED_UNICODE);
            die();
        }
        if ($this->model->existeSubtipoPorNombre($nombre)) {
            echo json_encode(['status' => "error", 'msg' => 'Ya existe un subtipo con ese nombre'], JSON_UNESCAPED_UNICODE);
            die();
        }
        if (!is_null($prefijo) && $this->model->existePrefijo($prefijo)) {
            echo json_encode(['status' => "error", 'msg' => 'Ese prefijo ya está en uso'], JSON_UNESCAPED_UNICODE);
            die();
        }
    ...
    public function actualizar()
    {
        ...
        if (empty($nombre) || empty($clave) || empty($id_tipo_operacion)) {
            echo json_encode(['status' => false, 'msg' => 'Nombre, clave y tipo de operación son obligatorios'], JSON_UNESCAPED_UNICODE);
            die();
        }

        if ($this->tipoRequierePuerto($id_tipo_operacion)) {
            if (empty($puerto_id_raw)) {
                echo json_encode(['status' => false, 'msg' => 'Este tipo requiere un puerto'], JSON_UNESCAPED_UNICODE);
                die();
            }
            $puerto_id = (int)$puerto_id_raw;
        } else {
        ...
        if ($this->model->existeSubtipoPorClave($clave, $id)) {
            echo json_encode(['status' => false, 'msg' => 'Ya existe un subtipo con esa clave'], JSON_UNESCAPED_UNICODE);
            die();
        }
        if ($this->model->existeSubtipoPorNombre($nombre, $id)) {
            echo json_encode(['status' => false, 'msg' => 'Ya existe un subtipo con ese nombre'], JSON_UNESCAPED_UNICODE);
            die();
        }
        if (!is_null($prefijo) && $this->model->existePrefijo($prefijo, $id)) {
            echo json_encode(['status' => false, 'msg' => 'Ese prefijo ya está en uso'], JSON_UNESCAPED_UNICODE);
            die();
        }
